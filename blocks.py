...
class Input:
    ...
    def sign(self, wallet):
        hash_string = '{}{}{}{}'.format(self.prev_tx_hash, self.output_index,
                        self.address, self.index).encode()
...
class Output:
    ...
    def __init__(self, address, amount, index=0):
        ...
        # Using input hash here to make output hash unique, especially for COINBASE tx
...
class Tx:
    ...
    def __init__(self, inputs, outputs, timestamps=None):
        self.inputs = inputs
        self.outputs = outputs
        self.timestamp = timestamps or int(time.time())
        self._hash = None
    ...
    def hash(self):
        ...
        inp_hash = sha256((str([el.as_dict for el in self.inputs]) + 
        ...
        hash_string = '{}{}{}'.format(
                [el.as_dict for el in self.inputs], [el.as_dict for el in self.outputs],
                self.timestamp
        )
    ...
    def from_dict(cls, data):
        ...
        inp_hash = sha256((str([el.as_dict for el in inps]) +
            str(data['timestamp'])).encode()).hexdigest()
